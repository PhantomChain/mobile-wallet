...
export class PhantomApiProvider {
  ...
  public phantomjs = require('phantomjscore');
  ...
  public setNetwork(network: StoredNetwork) {
    ...
    this.phantomjs.crypto.setNetworkVersion(this._network.version);
  ...
  private async findGoodPeerFromList(peerList: phantomts.Peer[]) {
    ...
    if (!this._network.isV2) {
      ...
      for (const peerId in peerConfigResponses) {
        ...
        const apiConfig = lodash.get(config, 'data.plugins["@phantomchain/core-api"]');
  ...
  public createTransaction(transaction: Transaction, key: string, secondKey: string, secondPassphrase: string): Observable<Transaction> {
    return Observable.create((observer) => {
      ...
      const keys = this.phantomjs.crypto.getKeys(key, jsNetwork);
      this.phantomjs.crypto.sign(transaction, keys);

      secondPassphrase = secondKey || secondPassphrase;

      if (secondPassphrase) {
        const secondKeys = this.phantomjs.crypto.getKeys(secondPassphrase, jsNetwork);
        this.phantomjs.crypto.secondSign(transaction, secondKeys);
      }

      transaction.id = this.phantomjs.crypto.getId(transaction);
  ...
  public postTransaction(transaction: phantomts.Transaction, peer: phantomts.Peer = this._network.activePeer, broadcast: boolean = true) {
    return Observable.create((observer) => {
      ...
      this._api.transaction.post(compressTransaction, peer).subscribe((result: phantomts.TransactionPostResponse) => {
